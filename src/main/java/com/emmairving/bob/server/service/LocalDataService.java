...
public class LocalDataService {
     ...
     * 处理RawLocalData数据
     *
     * @param start 需要处理的数据的起始编号，即上次处理数据的最后一条编号
     * @param user_id 数据拥有者
     */
    public void rawLocalDataHandler(int start, int user_id, String meter_number) {
        ...
        while (cur_cnt < cnt) {
            ...
            while( isFirstTime ) {
                // electricEnergy初始化
                if( rawLocalDataList.get(i).getElectric_energy() == null
     ...
     * @param rawLocalData
     */
    private void usualInsert(RawLocalData rawLocalData) {
        try {
            LocalData localData = new LocalData();
